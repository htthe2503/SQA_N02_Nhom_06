...
public class HoaDonDAOImpl implements HoaDonDAO{
    ...
    public List<HoaDon> getList() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int luongDienTieuThu = rs.getInt("LuongDienTieuThu");
                long tongTien = rs.getLong("TongTien");
                Date ngayLapPhieu = new Date(rs.getDate("NgayLapPhieu").getTime());
                boolean tinhTrang = rs.getBoolean("TinhTrang");
                int maNV = rs.getInt("MaNV");
                
                HoaDon hoaDon = new HoaDon(maHD, maKH, thangID, luongDienTieuThu, tongTien, ngayLapPhieu, tinhTrang, maNV);
                list.add(hoaDon);
            }
            ps.close();
    ...
    public int createHoaDon(HoaDon hoaDon) {
        try {
            Connection cons = MySQLConnect.getConnection();
            String sql = "INSERT INTO HoaDon(MaKH, ThangID, LuongDienTieuThu, TongTien, NgayLapPhieu, TinhTrang, MaNV) VALUES(?, ?, ?, ?, ?, ?, ?);";
            PreparedStatement ps = cons.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
            ps.setInt(1, hoaDon.getMaKH());
            ps.setInt(2, hoaDon.getThangID());
            ps.setInt(3, hoaDon.getLuongDienTieuThu());
    ...
    public List<HoaDon> getListOfThang(Thang thang) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                HoaDon hoaDon = new HoaDon(maHD, maKH, thang.getThangID(), luongDienTieuThu, tongTien, ngayLapPhieu, tinhTrang, maNV);
                list.add(hoaDon);
            }
            ps.close();
            connection.close();
